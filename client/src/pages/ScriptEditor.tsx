...
import { SceneVariation, ScriptCharacter } from "@/lib/types"; // ScriptCharacter is important
...
export default function ScriptEditor() {
  ...
  // --- NEW: Fetch all extracted characters for the current script ---
  const {
    data: allScriptCharacters = [],
    isLoading: isLoadingAllScriptCharacters,
  } = useQuery<ScriptCharacter[]>({
    queryKey: ["/api/scripts/extracted-characters", script?.id],
    queryFn: async ({ queryKey }) => {
      const [, sId] = queryKey as [string, number | undefined];
      if (!sId) return [];
      const res = await apiRequest(
        "GET",
        `/api/scripts/${sId}/extracted-characters`,
      );
      return res.json();
    },
    enabled: !!script?.id,
    staleTime: Infinity, // These are unlikely to change unless script content changes
  });
  // --- END NEW ---
  ...
  const reanalyzeScriptMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ // Also invalidate extracted characters on re-analyze
        queryKey: ["/api/scripts/extracted-characters", script?.id],
      });
      if (activeSceneId) {
        queryClient.invalidateQueries({
        ...
        setVideoGenerationStates((prev) => {
          ...
          switch (data.status) {
            ...
            case "processing":
              ...
              if (data.logs) {
                if (
                  data.logs.toLowerCase().includes("frame generation")
                )
                  progress = 50;
                if (data.logs.toLowerCase().includes("upscaling"))
                  progress = 75;
              }
  ...
  const startPollingPrediction = useCallback(
    (predictionId: string, variationId: number) => {
      if (pollingIntervals.current[predictionId]) return;
      setTimeout(() => pollPredictionStatus(predictionId, variationId), 2000);
      pollingIntervals.current[predictionId] = setInterval(
        () => pollPredictionStatus(predictionId, variationId),
        5000,
      );
  ...
  const handleExport = async () => {
    console.log("Export triggered");
    setIsExporting(true); setIsExporting(false);
  };
  const handleSceneSelect = (sceneId: number) => {
    if (activeSceneId !== sceneId) setActiveSceneId(sceneId);
  };
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-lg shadow p-4">
                  ...
                  {/* --- MODIFIED CharacterCasting props --- */}
                  <CharacterCasting
                    scriptId={script?.id || null}
                    isLoadingScript={isLoadingScript}
                    allScriptCharacters={allScriptCharacters}
                    isLoadingAllScriptCharacters={isLoadingAllScriptCharacters}
                    filmGenre={filmGenreForCasting}
                    projectBudgetTier={projectBudgetTierForCasting}
                  />
                  {/* --- END MODIFIED CharacterCasting props --- */}
                </div>
              </div>
            </>
          )}
