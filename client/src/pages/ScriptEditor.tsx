...
export default function ScriptEditor() {
  ...
  useEffect(() => {
    if (script?.id) {
      console.log(`[ScriptEditor] New script context (ID: ${script.id}). Resetting local states and script-specific queries.`);
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/scripts/scene-variations'] }); // General invalidation for all variations
      queryClient.invalidateQueries({ queryKey: ['/api/scenes/suggest-locations'] });
      queryClient.invalidateQueries({ queryKey: ['/api/characters/suggest-actors'] });
    }
   ...
   const startVideoGenerationMutation = useMutation({
       ...
       onSuccess: (result) => {
           ...
           if (responseData.predictionId && responseData.status && !['failed', 'canceled'].includes(responseData.status)) {
               setVideoGenerationStates(prev => ({
                   ...prev,
                   [variationId]: { status: responseData.status as VideoGenerationStatus, predictionId: responseData.predictionId, error: null, videoUrl: null, progress: 20, stageMessage: "Video job started..." }
               }));
               startPollingPrediction(responseData.predictionId, variationId);
               toast({ title: "Video Generation Started", description: `Processing variation ${variationId}...` });
           } else {
