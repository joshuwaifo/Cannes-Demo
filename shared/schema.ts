...
export const ProductCategory = {
  ...
  FASHION: "FASHION",
  WATCH: "WATCH",
  // Add OTHER if it's not already present, or ensure it's handled
  // OTHER: "OTHER", 
} as const;
...
export const scenes = pgTable("scenes", {
  ...
  // VFX Analysis Fields - START
  is_vfx_scene: boolean("is_vfx_scene").default(false).notNull(),
  vfx_description: text("vfx_description"), // Nullable
  vfx_keywords: jsonb("vfx_keywords").$type<string[] | null>(), // Nullable array of strings
  // VFX Analysis Fields - END
...
export const scenesRelations = relations(scenes, ({ one, many }) => ({
  ...
  variations: many(sceneVariations),
  // We'll add relation to vfx_scene_details later
}));

export const insertSceneSchema = createInsertSchema(scenes, {
  // If new fields need specific zod validation for inserts, add here
  // For now, defaults and nullability handle it for basic inserts
...
// --- Scene Variations ---
// ... (existing sceneVariations schema)
export const sceneVariations = pgTable("scene_variations", {
  id: serial("id").primaryKey(),
  sceneId: integer("scene_id")
    .references(() => scenes.id, { onDelete: "cascade" })
...
// --- Script Generation Form ---
// ... (existing scriptGenerationFormSchema)
export const scriptGenerationFormSchema = z.object({
  projectTitle: z.string().min(1, "Project Title is required."),
  logline: z
    .string()
