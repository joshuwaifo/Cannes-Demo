...
const MODEL_NAME = "gemini-2.5-flash-preview-04-17"; // Using "latest" often points to a recent stable version for Flash
...
const defaultTools: SDKTool[] = [{ googleSearch: {} }]; // Enable Google Search if needed by prompts
...
export async function suggestActorsForCharacterViaGemini(
    scriptContent: string, // Kept for broader context, though character profile is primary
    characterToCast: ExtractedCharacter, // This is BackendExtractedCharacter
    ...
    const characterProfiledGender = characterToCast.gender || "Unknown";

    // Use the gender from UI criteria if specified and valid, otherwise default to character's profiled gender for the AI prompt
    const targetGenderForPrompt =
        criteria.gender &&
        criteria.gender.toLowerCase() !== "any" &&
        criteria.gender.toLowerCase() !== "all" &&
        criteria.gender.toLowerCase() !== "unknown"
            ? criteria.gender
            : characterProfiledGender;

    const logPrefix = `[Gemini Actor Suggest for "${characterNameToCast}" (Age: ${characterEstimatedAge}, Target Gender: ${targetGenderForPrompt})]`;
...
The PROVIDED_ACTOR_LIST has already been pre-filtered for basic age and gender compatibility. Focus on artistic fit, role interpretation, and strategic considerations.

CHARACTER_PROFILE:
- Name: "${safeCharacterName}"
- Estimated Age Range: "${characterEstimatedAge}"
- Profiled Gender (from script analysis): "${characterProfiledGender}"
- Target Casting Gender (primary gender to match from list): "${targetGenderForPrompt}"
...
PROVIDED_ACTOR_LIST (Only select from this list):
${formattedActorsString}

INSTRUCTIONS:
1.  Strictly select actors ONLY from the PROVIDED_ACTOR_LIST. Do NOT suggest actors outside this list.
2.  Prioritize actors whose actual age (derived from their DOB in the list) closely matches the character's ESTIMATED_AGE_RANGE.
3.  Crucially, ensure the selected actor's GENDER (from the list) matches the TARGET_CASTING_GENDER ("${targetGenderForPrompt}"). If Target Casting Gender is 'Any' or 'Unknown', then Profiled Gender can be a secondary guide but artistic fit and age are more important.
4.  For each chosen actor, provide:
    - "actorName": (EXACTLY as it appears in the PROVIDED_ACTOR_LIST).
    - "matchReason": (1-2 concise sentences. Explain why this actor is a strong fit for the CHARACTER_PROFILE and how they align with the USER_CRITERIA, especially age and gender. If there's a slight age variance, briefly justify if other factors strongly compensate).
...
5.  Return exactly ${numberOfSuggestions} suggestions if possible, or fewer if suitable matches are limited within the list.
...
Ensure the output is a single, clean JSON object without any surrounding text or markdown.`;
        ...
        console.log(
            `${logPrefix} Sending request to Gemini with ${availableActorsFromDb.length} pre-filtered actors.`,
        );
