...
export function sanitizeForSafetyFilter(text: string): string {
    if (!text) return "";
    const profanityMap: Record<string, string> = {
        'shit': 'stuff', 'damn': 'darn', 'fuckin\'': 'really',
        'fucking': 'really', 'hell': 'heck',
    };
    let sanitized = text;
    for (const word in profanityMap) {
        const regex = new RegExp(`\\b${word}\\b`, 'gi');
  ...
  targetBrandableSceneCount: number = 5,
    ...
    try {
        ...
        const model = genAI.getGenerativeModel({
            ...
            generationConfig: { responseMimeType: "application/json", temperature: 0.3, maxOutputTokens: 4096 } as GenerationConfig,
        });
        const scenesTextForPrompt = scenes.map((scene) => {
            const safeHeading = sanitizeForSafetyFilter(scene.heading);
            const safeContent = sanitizeForSafetyFilter(scene.content.substring(0, 500));
            return `SCENE_ID: ${scene.id}\nHEADING: ${safeHeading}\nCONTENT_SNIPPET: ${safeContent}...\n---`;
        }).join("\n");
        ...
        try {
            const parsedResponse: { brandableScenes?: BrandableSceneAnalysis[] } = JSON.parse(responseText);
            if (!parsedResponse.brandableScenes || !Array.isArray(parsedResponse.brandableScenes)) {
                console.error("[Gemini Analysis] Response not in expected format. Raw:", responseText);
                return { brandableScenes: [] };
            }
            const sceneIdMap = new Map(scenes.map(s => [s.id, true]));
            const validCategories = new Set(Object.values(ProductCategory));
            const validatedBrandableScenes = parsedResponse.brandableScenes.filter((bs, index) => {
                const sceneId = typeof bs.sceneId === 'string' ? parseInt(bs.sceneId, 10) : bs.sceneId;
                if (isNaN(sceneId) || !sceneIdMap.has(sceneId)) { console.warn(`[Gemini Val] Invalid sceneId ${bs.sceneId} at ${index}.`); return false; }
                if (typeof bs.reason !== 'string' || bs.reason.length < 10) { console.warn(`[Gemini Val] Invalid reason for ${sceneId} at ${index}.`); return false; }
                if (!Array.isArray(bs.suggestedProducts) || bs.suggestedProducts.length === 0 || bs.suggestedProducts.length > 3) { console.warn(`[Gemini Val] Invalid suggestedProducts for ${sceneId} at ${index}.`); return false; }
                if (!bs.suggestedProducts.every(p => validCategories.has(p as ProductCategory))) { console.warn(`[Gemini Val] Invalid category in suggestedProducts for ${sceneId} at ${index}.`); return false; }
                bs.sceneId = sceneId;
                bs.suggestedProducts = bs.suggestedProducts.filter(p => validCategories.has(p as ProductCategory));
                return true;
            }).slice(0, targetBrandableSceneCount);
            console.log(`[Gemini Analysis] Identified ${validatedBrandableScenes.length} valid brandable scenes.`);
            return { brandableScenes: validatedBrandableScenes };
        } catch (parseError) {
            console.error("[Gemini Analysis] Failed to parse JSON:", parseError, "\nRaw:", responseText);
            return { brandableScenes: [] };
        }
    } catch (error: any) {
        console.error("[Gemini Analysis] Error analyzing scenes:", error.message || error);
        return { brandableScenes: [] };
    }
}
...
export async function generateCreativePlacementPrompt(scene: Scene, product: Product): Promise<string> {
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
        model: "gemini-1.5-flash", safetySettings,
        generationConfig: { responseMimeType: "text/plain", temperature: 0.7, maxOutputTokens: 150, stopSequences: ["\n\n", "---"] } as GenerationConfig
    });
    const safeSceneContextSummary = sanitizeForSafetyFilter(scene.content?.substring(0, 500) || "No scene content provided.");
    const safeHeading = sanitizeForSafetyFilter(scene.heading);
    const safeReason = sanitizeForSafetyFilter(scene.brandableReason || 'N/A');
    ...
    console.log(`${logPrefix} Prompting Gemini...`);
    const result = await model.generateContent(promptToGemini);
    let generatedPrompt = result.response.text().trim().replace(/^["']|["']$/g, '').replace(/^Prompt:\s*/i, '').split('\n')[0];
    if (!generatedPrompt) { console.warn(`${logPrefix} Empty prompt. Using fallback.`); return fallbackPrompt; }
    const approxTokens = generatedPrompt.split(/\s+/).filter(Boolean).length;
    if (approxTokens > 110) { console.warn(`${logPrefix} Prompt too long (${approxTokens} words), truncating.`); generatedPrompt = generatedPrompt.split(/\s+/).slice(0, 100).join(' ') + '...'; }
    console.log(`${logPrefix} Received prompt: ${generatedPrompt.substring(0, 100)}...`);
    return generatedPrompt;
  } catch (error: any) {
    console.error(`${logPrefix} Failed to generate creative prompt:`, error);
...
// --- NEW: Character Extraction with Gemini ---
...
export async function extractCharactersWithGemini(scriptContent: string): Promise<ExtractedCharacter[]> {
  ...
  console.log(`${logPrefix} Starting character extraction from script content (length: ${scriptContent.length}).`);
  ...
  try {
    ...
    // Using a slightly smaller substring for the prompt itself, ensuring it doesn't hit any internal limits.
    // Max context for gemini-1.5-flash is 1M tokens (~700k chars for English text).
    // We'll use a safe limit here for the script content part of the prompt.
    const maxScriptCharsForPrompt = 650000; 
    const scriptContentForPrompt = scriptContent.length > maxScriptCharsForPrompt 
        ? scriptContent.substring(0, maxScriptCharsForPrompt) + "\n...[TRUNCATED FOR PROMPT]..." 
        : scriptContent;

    const prompt = `
    You are an expert script analyst. Your task is to analyze the provided screenplay content and extract all unique speaking character names.

    Instructions:
    1.  Carefully read the screenplay. Identify names that are clearly designated as characters, typically appearing in ALL CAPS before lines of dialogue, or explicitly described as characters in action lines.
    2.  The list of character names must be unique. Do not repeat names.
    3.  Exclude common non-character, all-caps elements like "V.O." (Voice Over), "O.S." (Off Screen), "CONT'D", "CONTINUED", scene headings (e.g., "INT. ROOM - DAY"), parentheticals (e.g., "(beat)", "(laughing)"), and general capitalized action descriptions or sound effects (e.g., "CLOSE UP ON PHONE", "A LOUD BANG").
    4.  Focus on names that would appear in a cast list. Character names are typically short (1-3 words).
    5.  Return the result STRICTLY as a single JSON object. This JSON object must have a single key "characters".
    6.  The value of "characters" must be an array of objects. Each object in the array must have a single key "name", and its value should be the extracted character name (string, in ALL CAPS).
    7.  If no distinct characters are found, return an empty array for "characters".
    8.  Do NOT include any explanatory text, markdown formatting, or anything else before or after the JSON object. Just the JSON.

    Example of desired JSON output if characters "JOHN", "MARY", and "DR. SMITH" are found:
    {
      "characters": [
        {"name": "JOHN"},
        {"name": "MARY"},
    ...
    console.log(`${logPrefix} Sending request to Gemini for character extraction...`);
    const result = await model.generateContent(prompt);
    const response = await result.response;
    const responseText = response.text(); 

    // console.log(`${logPrefix} Raw Gemini character response:`, responseText); // For debugging
    ...
    try {
      ...
      if (!parsedResponse.characters || !Array.isArray(parsedResponse.characters)) {
        console.error(`${logPrefix} Gemini response is not in the expected format (missing or invalid 'characters' array). Raw text:`, responseText);
        return [];
      }

      const extractedCharacters = parsedResponse.characters
        .map(charObj => ({ name: (charObj.name || "").toString().trim().toUpperCase() }))
      ...
      console.log(`${logPrefix} Successfully extracted ${extractedCharacters.length} unique characters.`);
      return extractedCharacters;

    } catch (parseError) {
      console.error(`${logPrefix} Failed to parse Gemini JSON response for characters:`, parseError, "\nRaw Response Text:", responseText);
      // Attempt to extract from text if JSON parsing fails and it looks like a list
      if (!responseText.trim().startsWith("{")) {
          const lines = responseText.split('\n').map(l => l.trim().toUpperCase().replace(/[^A-Z0-9\s'-]/g, ''));
          const potentialChars = lines.filter(l => l && l.length > 1 && l.length < 50 && !l.startsWith("SCENE") && !l.startsWith("INT") && !l.startsWith("EXT"));
          if (potentialChars.length > 0) {
              console.warn(`${logPrefix} Attempting fallback text parsing for characters.`);
              return potentialChars.map(name => ({name})).filter((charObj, index, self) => self.findIndex(c => c.name === charObj.name) === index);
          }
      }
      return [];
    }
    ...
    console.error(`${logPrefix} Error extracting characters with Gemini:`, error.message || error);
