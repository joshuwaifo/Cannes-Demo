...
//     generateCreativePlacementPrompt,
//     AIAnalysisResponseForRoutes,
//     extractCharactersWithGemini, // This now returns richer ScriptCharacter[]
// } from "./services/file-upload-service";
// import {
//     suggestActorsForCharacterViaGemini,
//     ActorAISuggestion,
...
// import { z } from "zod";
// import {
//     insertProductSchema, insertActorSchema, insertLocationSchema, ProductCategory,
//     Product as DbProduct, Scene as DbScene, Actor as DbActor, Location as DbLocation,
//     SceneVariation as DbSceneVariationSchema, Product, Script,
// } from "@shared/schema";
// import { ZodError } from "zod";
// import {
//     ActorSuggestion as ClientActorSuggestion,
//     ScriptCharacter, // Use this for the new endpoint response type
//     ClientSuggestedLocation,
// } from "../../client/src/lib/types";

// interface SceneVariation extends DbSceneVariationSchema {
//     productName?: string;
//     productCategory?: ProductCategory;
//     productImageUrl?: string | null;
// }
// const sanitizeString = (str: string | null | undefined): string => { if (!str) return ""; return str.replace(/\u0000/g, "").replace(/[^\x20-\x7E\u00A0-\uFFFF]/g, ""); };
// const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 15 * 1024 * 1024 }, fileFilter: (req, file, cb) => { const allowedTypes = ["application/pdf", "image/jpeg", "image/png", "image/jpg"]; if (allowedTypes.includes(file.mimetype)) { cb(null, true); } else { cb(new Error("Invalid file type. Only PDF, JPG, PNG allowed.")); } },});
// interface CacheEntry<T> { data: T; timestamp: number;}
// const aiSuggestionCache = new Map<string, CacheEntry<any>>();
// const CACHE_TTL = 15 * 60 * 1000;
// function getFromCache<T>(key: string): T | null { const entry = aiSuggestionCache.get(key); if (entry && (Date.now() - entry.timestamp < CACHE_TTL)) { return entry.data as T;} aiSuggestionCache.delete(key); return null;}
// function setInCache<T>(key: string, data: T): void { aiSuggestionCache.set(key, { data, timestamp: Date.now() });}
// async function _generateAndSaveSceneVariationsForRoute(sceneId: number): Promise<SceneVariation[]> { const logPrefix = `[VarGen Route S:${sceneId}]`; try { let scene = await storage.getSceneById(sceneId); if (!scene) { console.error(`${logPrefix} Scene not found.`); return []; } if (!scene.isBrandable || !scene.suggestedCategories || scene.suggestedCategories.length === 0) { try { const analysisResult = await identifyBrandableScenesWithGemini([scene], 1); if (analysisResult.brandableScenes.length > 0 && analysisResult.brandableScenes[0].sceneId === scene.id) { const brandableData = analysisResult.brandableScenes[0]; const updatedSceneInDb = await storage.updateScene(scene.id, { isBrandable: true, brandableReason: brandableData.reason, suggestedCategories: brandableData.suggestedProducts }); if (updatedSceneInDb) scene = updatedSceneInDb; } else { if (!scene.isBrandable) { const updatedSceneInDb = await storage.updateScene(scene.id, { isBrandable: true }); if (updatedSceneInDb) scene = updatedSceneInDb; } } } catch (analysisError) { console.error(`${logPrefix} Error during on-demand scene analysis:`, analysisError); } } const categories = scene.suggestedCategories || []; const selectedProducts = await storage.getTopMatchingProductsForScene(sceneId, categories, 3); if (selectedProducts.length === 0) return []; const variationPromises = selectedProducts.map(async (product, i) => { const variationNumber = i + 1; const varLogPrefixV = `${logPrefix} V${variationNumber} (P:${product.id})`; try { const creativePrompt = await generateCreativePlacementPrompt(scene!, product); if (!creativePrompt || creativePrompt.includes("Error:")) { throw new Error(`Gemini prompt error: ${creativePrompt}`); } const generationResult = await generateProductPlacement({ scene: scene!, product, variationNumber, prompt: creativePrompt }); const cleanDescription = `Variation ${variationNumber}: ${product.name} in ${scene!.heading}. Prompt used: ${creativePrompt.substring(0, 50)}...`; const variationData = { sceneId: scene!.id, productId: product.id, variationNumber, description: sanitizeString(cleanDescription), imageUrl: generationResult.imageUrl, geminiPrompt: creativePrompt, isSelected: false, }; const newVariationDb = await storage.createSceneVariation(variationData); return { ...newVariationDb, productName: product.name, productCategory: product.category, productImageUrl: product.imageUrl } as SceneVariation; } catch (error) { console.error(`${varLogPrefixV} Error in variation generation:`, error); return null; } }); const results = await Promise.all(variationPromises); return results.filter((v): v is SceneVariation => v !== null); } catch (outerError) { console.error(`${logPrefix} CRITICAL ERROR in on-demand variation generation process:`, outerError); return []; } }

// export async function registerRoutes(app: Express): Promise<Server> {
//     const apiPrefix = "/api";
//     // Standard CRUD routes (Actors, Products, Locations, Scripts, Scenes, Variations)
...
//     app.get(`${apiPrefix}/scripts/:scriptId/suggest-locations`, async (req, res, next) => { const scriptIdParam = req.params.scriptId; const budgetParam = req.query.budget as string; const countParam = req.query.count as string; const logPrefix = `[Route Script Location Suggestion for Script ID:${scriptIdParam}]`; try { const scriptId = parseInt(scriptIdParam); if (isNaN(scriptId)) return res.status(400).json({ message: "Valid Script ID is required" }); const script = await storage.getScriptById(scriptId); if (!script || !script.content) { return res.status(404).json({ message: "Script not found or has no content" });} const projectBudget = budgetParam ? parseInt(budgetParam) : undefined; const numberOfSuggestions = countParam ? parseInt(countParam) : 5; const allDbLocations = await storage.getAllLocationsForAISuggestion(); if (allDbLocations.length === 0) { console.log(`${logPrefix} No locations in DB to suggest from.`); return res.json([]);} const aiLocationSuggestions: LocationAISuggestion[] = await suggestLocationsForScriptViaGemini(script.content, script.id, allDbLocations, projectBudget, numberOfSuggestions); if (aiLocationSuggestions.length === 0) return res.json([]); const finalSuggestedLocations: ClientSuggestedLocation[] = []; for (const aiSugg of aiLocationSuggestions) { const locationDetails = await storage.getLocationById(aiSugg.locationId); if (locationDetails) { finalSuggestedLocations.push({ ...locationDetails, matchReason: aiSugg.matchReason, estimatedIncentiveValue: aiSugg.estimatedIncentiveNotes, confidenceScore: aiSugg.confidenceScore });} else { console.warn(`${logPrefix} Location ID "${aiSugg.locationId}" suggested by AI not found in DB.`);} } res.json(finalSuggestedLocations); } catch (error) { console.error(`${logPrefix} Error:`, error); next(error);} });


//     // --- Endpoint for ALL extracted characters (with profiles) from a script ---
//     app.get(`${apiPrefix}/scripts/:scriptId/extracted-characters`, async (req, res, next) => {
//         const scriptIdParam = req.params.scriptId;
//         const logPrefix = `[ExtractedChars Route for Script ID:${scriptIdParam}]`;
//         try {
//             const scriptId = parseInt(scriptIdParam);
...
//             const script = await storage.getScriptById(scriptId);
//             if (!script || !script.content) {
//                 console.warn(`${logPrefix} Script not found or has no content.`);
//                 return res.status(404).json({ message: "Script not found or has no content" });
//             }

//             const cacheKey = `scriptCharactersProfile:${scriptId}`;
//             const cachedCharacters = getFromCache<ScriptCharacter[]>(cacheKey);
//             if (cachedCharacters) {
//                 console.log(`${logPrefix} Serving extracted character profiles from cache.`);
//                 return res.json(cachedCharacters);
//             }

//             console.log(`${logPrefix} Script content found. Extracting character profiles...`);
//             const characters: ScriptCharacter[] = await extractCharactersWithGemini(script.content); // This now returns richer profiles
//             console.log(`${logPrefix} Extracted ${characters.length} character profiles.`);

//             setInCache(cacheKey, characters);
//             res.json(characters);
//         } catch (error) {
//             console.error(`${logPrefix} Error:`, error);
//             next(error);
...
//     // --- MODIFIED: /api/characters/:characterName/suggest-actors ---
//     app.get(`${apiPrefix}/characters/:characterName/suggest-actors`, async (req, res, next) => {
//         const characterName = req.params.characterName;
//         const {
//             scriptId: scriptIdQueryParam,
...
//         const logPrefix = `[SuggestActors Route for "${characterName}" in Script ${scriptIdQueryParam}]`;

//         try {
//             if (!characterName) return res.status(400).json({ message: "Character name required" });
//             if (!scriptIdQueryParam) return res.status(400).json({ message: "Script ID query parameter is required" });

//             const scriptId = parseInt(scriptIdQueryParam);
//             if (isNaN(scriptId)) return res.status(400).json({ message: "Valid Script ID is required" });

//             // Fetch all enriched character profiles for the script (uses cache internally if available)
//             const allCharsCacheKey = `scriptCharactersProfile:${scriptId}`;
//             let allCharactersInScript = getFromCache<ScriptCharacter[]>(allCharsCacheKey);
//             if (!allCharactersInScript) {
//                 const scriptToUse = await storage.getScriptById(scriptId);
//                 if (!scriptToUse || !scriptToUse.content) {
//                     return res.status(404).json({ message: `Script with ID ${scriptId} not found or is empty.` });
//                 }
//                 console.log(`${logPrefix} All character profiles for script ${scriptId} not in cache, extracting...`);
//                 allCharactersInScript = await extractCharactersWithGemini(scriptToUse.content);
//                 setInCache(allCharsCacheKey, allCharactersInScript);
//             } else {
//                  console.log(`${logPrefix} All character profiles for script ${scriptId} found in cache.`);
//             }

//             const characterToCastDetails = allCharactersInScript.find(c => c.name === characterName.toUpperCase());

//             if (!characterToCastDetails) {
//                 console.warn(`${logPrefix} Character "${characterName}" not found among extracted profiles for script ID ${scriptId}.`);
//                 return res.status(404).json({ message: `Character "${characterName}" not found in script.` });
//             }

//             // Determine final criteria for AI, prioritizing AI-derived profile, then UI, then defaults
//             const finalFilmGenre = filmGenreFromUI || "Any"; // Assuming film genre is mostly a UI preference for now
//             const finalRoleType = roleTypeFromUI || characterToCastDetails.roleType || "Unknown";
//             const finalBudgetTier = budgetTierFromUI || characterToCastDetails.recommendedBudgetTier || "Any";
//             const characterGenderForFilter = characterToCastDetails.gender !== "Unknown" ? characterToCastDetails.gender : undefined;


//             const suggestionCacheKey = `suggestActors:${scriptId}:${characterName}:${finalFilmGenre}:${finalRoleType}:${finalBudgetTier}:${characterToCastDetails.estimatedAgeRange}:${characterGenderForFilter}`;
//             const cachedSuggestions = getFromCache<ClientActorSuggestion[]>(suggestionCacheKey);
//             if (cachedSuggestions) {
//                 console.log(`${logPrefix} Serving actor suggestions from cache.`);
//                 return res.json(cachedSuggestions);
//             }
//             console.log(`${logPrefix} Actor suggestions not in cache, processing...`);


//             let minBirthYear: number | undefined;
//             let maxBirthYear: number | undefined;
//             const currentYear = new Date().getFullYear();

//             if (characterToCastDetails.estimatedAgeRange) {
//                 const ageRange = characterToCastDetails.estimatedAgeRange;
//                 const ageParts = ageRange.match(/\d+/g);
//                 if (ageParts && ageParts.length === 1) { const age = parseInt(ageParts[0]); if(!isNaN(age)){ minBirthYear = currentYear - age - 7; maxBirthYear = currentYear - age + 7;}}
//                 else if (ageParts && ageParts.length >= 2) { const minAge = parseInt(ageParts[0]); const maxAge = parseInt(ageParts[ageParts.length -1 ]); if (!isNaN(minAge) && !isNaN(maxAge)) { minBirthYear = currentYear - maxAge - 5; maxBirthYear = currentYear - minAge + 5;}}
//                 else if (ageRange.toLowerCase().includes("teen")) { minBirthYear = currentYear - 19 - 2; maxBirthYear = currentYear - 13 + 2;}
//                 else if (ageRange.toLowerCase().includes("child")) { minBirthYear = currentYear - 12 - 2; maxBirthYear = currentYear - 5 + 2;}
//             }

//             const preFilteredActors = await storage.getActorsForAISuggestionByCriteria({
//                 minBirthYear, maxBirthYear,
//                 gender: characterGenderForFilter,
//                 // Future: Add role type and budget tier to actors table and filter here if desired
//                 limit: 60 // Further reduced limit of actors to send to AI
//             });

//             if (preFilteredActors.length === 0) {
//                 console.log(`${logPrefix} No actors after pre-filtering based on character profile. Cache empty response.`);
//                 setInCache(suggestionCacheKey, []);
//                 return res.json([]);
//             }
//             console.log(`${logPrefix} Pre-filtered ${preFilteredActors.length} actors for AI.`);

//             const characterContextForAI = characterToCastDetails.description || `A character named ${characterToCastDetails.name}. Age: ${characterToCastDetails.estimatedAgeRange}. Role: ${characterToCastDetails.roleType}.`;

//             const aiSuggestions: ActorAISuggestion[] = await suggestActorsForCharacterViaGemini(
//                 characterContextForAI,
//                 characterToCastDetails, // Pass the rich profile
//                 preFilteredActors,
//                 { filmGenre: finalFilmGenre, roleType: finalRoleType, budgetTier: finalBudgetTier },
//                 5
//             );

//             const finalSuggestions: ClientActorSuggestion[] = [];
//             for (const aiSugg of aiSuggestions) {
//                 const actorDetails = preFilteredActors.find(a => a.name === aiSugg.actorName);
//                 if (actorDetails) {
//                     finalSuggestions.push({ ...actorDetails, matchReason: aiSugg.matchReason, controversyLevel: aiSugg.controversyLevel });
//                 } else {
//                     console.warn(`${logPrefix} Actor "${aiSugg.actorName}" suggested by AI not found in pre-filtered list (should not happen).`);
//                 }
//             }

//             setInCache(suggestionCacheKey, finalSuggestions);
//             res.json(finalSuggestions);
//         } catch (error) {
//             console.error(`${logPrefix} Error processing suggest-actors request:`, error);
//             next(error);
//         }
//     });
